...
import com.webdev.spring.repository.ReplyRepository;
...
public class ReplyServiceImpl implements ReplyService{
    ...
    public void remove(Long rno) {
        // If the entity is not found in the persistence store it is silently ignored.: 해당 객체가 없을 경우, 예외 발생 없이 무시된다. 버전이 변경되어 예외 발생이 사라졌다.
        // replyRepository.deleteById(rno);

        Optional<Reply> replyOptional = replyRepository.findById(rno);

        Reply reply = replyOptional.orElseThrow(() -> new NoSuchElementException());
